...
function getVideos(req, res, next) {
  try {
    ...
    if (visitorId.length > 128)
      throw createHttpError(400, "visitorId is too long");
...
function toggleLike(req, res, next) {
  try {
    ...
    if (typeof videoId !== "string" || !videoId.trim())
      throw createHttpError(400, "videoId is required");
    if (
      visitorId !== undefined &&
      ...
      throw createHttpError(
        400,
        "visitorId must be a non-empty string of at most 128 characters",
      );
    }
    ...
    return successResponse(res, {
      data: toggleVideoLike(video, getVisitorId(visitorId, req.ip)),
    });
  } catch (error) {
    return next(error);
  }
...
function shareVideo(req, res, next) {
  try {
    ...
    if (typeof platform !== "string")
      throw createHttpError(400, "platform is required");

    const video = findVideo(videoId);
    if (!video) throw createHttpError(404, "Video not found");
    if (!sharePlatforms.has(platform))
      throw createHttpError(400, "Invalid share platform");
